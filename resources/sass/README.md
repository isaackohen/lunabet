<p align="center">
  <img src="resources/img/misc/datagamble_logo_black.png" alt="Datagamble Logo">
</p>
<p align="center">
...
Generation process is simple: run `php artisan Datagamble:keys`.

If you intend to use NowPayments service, backup `NowPayments_PASSPHRASE` from `.env` to somewhere so you won't lose access to NowPayments wallet by some accident.
...
Finally, run `Datagamble:subscribe`:
```
...
**1.** Install NowPaymentsJS:
```
cd NowPaymentsJS/modules/express
npm ci
```

**Note:** We do not recommend installing NowPayments Express as root, but if you need to do so, you must run `npm ci --unsafe-perm` for the last step instead of `npm ci`.

### Running NowPayments Express

From the express module folder (`modules/express`), run this command:

`npm run start`

You should see NowPayments Express start up in the default test environment:

```
NowPayments-Express running
Environment: test
...
When running NowPayments Express against the NowPayments production environment using real funds, you should make sure the `NODE_ENV` environment variable is set to `production`. This will turn off some debugging information which could leak information about the system which is running NowPayments Express. If an unsafe configuration is detected, NowPayments Express will emit a warning upon startup. In a future version of NowPayments Express, this will turn into a hard error and NowPayments Express will fail to start up.

Additionally, when running against the production env and listening on external interfaces, NowPayments Express must be run with TLS enabled by setting the `keyPath` and `crtPath` configuration options, otherwise NowPayments Express will error upon startup with the following message:
...
We strongly recommend always enabling TLS when running against the NowPayments production environment. However, if you must opt out of this requirement, you may do so by setting the `disableSSL` configuration option. Use at your own risk, as this may allow a man-in-the-middle to access sensitive information as it is sent over the wire in cleartext.

### Webhook

Webhook (`https://<url>/api/NowPaymentsWebhook`) is created automatically for NowPayments wallets.
...
#### Skip this step if you will run nodes and Datagamble on same server.
...
Make sure that WS server (`php artisan Datagamble:subscribe` with `laravel-echo-server`) are running.
...
* Never run ```php artisan Datagamble:subscribe``` twice. If you need to restart it, kill previous process, otherwise events will be processed twice - chat will duplicate messages, users could dupe their balance.
...
* Some currencies are supported by NowPayments and some are supported as native. While most of the currencies share user balances, we recommend to pick something once and never change it, because if you remove Bitcoin SV and switch to local nodes completely - your users will lose their funds.
