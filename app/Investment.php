...
class Investment extends Model {
    ...
    public static function getUserProfit(\App\Currency\Currency $currency, $created_at, $user = null) {
        $user = $user == null ? '*' : $user->_id;
        $usersWon = DB::table('games')->where('created_at', '>=', $created_at)->where('demo', '!=', true)->where('currency', $currency->id())->where('status', 'win')->where('user', $user)->sum('profit');
        $usersWonWager = DB::table('games')->where('created_at', '>=', $created_at)->where('demo', '!=', true)->where('currency', $currency->id())->where('status', 'win')->where('user', $user)->sum('wager');
        $usersLost = DB::table('games')->where('created_at', '>=', $created_at)->where('demo', '!=', true)->where('currency', $currency->id())->where('status', 'lose')->where('user', $user)->sum('wager');
        return $usersLost - ($usersWon - $usersWonWager);
    }
