...
class VideoPoker extends ExtendedGame implements MultiplierCanBeLimited {
    ...
    private function getMultiplier(Game $game, ?array $turnData, bool $replace, string $server_seed) {
        ...
        if(in_array('A', $userValues) && in_array('K', $userValues) && in_array('Q', $userValues) && in_array('J', $userValues) && in_array('10', $userValues)) return 600;
        else if($isStraight && $isFlush) return 60;
        else if($fours == 1) return 22;
        else if($triplets == 1 && $pairs == 1) return 9;
        else if($isFlush) return 6;
        else if($isStraight) return 4;
        else if($triplets == 1) return 3;
